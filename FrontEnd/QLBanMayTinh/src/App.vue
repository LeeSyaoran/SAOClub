...
const sidebarCatsBase = computed(() => [
  { id: 'office',   icon: "💻", name: t('home.sidebar.office'),   keywords: ["van_phong", "sinh_vien", "văn phòng", "sinh viên"] },
  { id: 'gaming',   icon: "🎮", name: t('home.sidebar.gaming'),   keywords: ["gaming"] },
  { id: 'graphics', icon: "⚡", name: t('home.sidebar.graphics'), keywords: ["do_hoa", "ky_thuat", "đồ họa", "kỹ thuật"] },
  { id: 'macbook',  icon: "🍎", name: t('home.sidebar.macbook'),  keywords: ["macbook", "apple"] },
  { id: 'used',     icon: "⭐", name: t('home.sidebar.used'),     keywords: ["cu", "gia_re", "cũ", "rẻ"] },
  { id: 'parts',    icon: "🔧", name: t('home.sidebar.parts'),    keywords: ["linh_kien", "ram", "ssd", "linh kiện"] },
]);
...
const onPopState = (event) => {
  ...
  if (state?.view === "product") {
    ...
    selectedProduct.value = products.value.find(p => p.bienTheId === state.bienTheId) || null;
  } else {
...
const handleOrderPlaced = () => { cart.value = []; };
...
<template>
  ...
  <div>
    ...
    <section v-else-if="isAdminHash && !auth.isAdmin"
      ...
      <div class="text-center d-flex flex-column align-items-center gap-3" style="color:var(--text-primary);">
        <div style="font-size:3rem;">🔒</div>
        <h2 class="fw-black mb-0" style="font-size:1.5rem;">{{ t('adminAccess.title') }}</h2>
        <p class="mb-0" style="color:var(--text-secondary);">{{ t('adminAccess.desc') }}</p>
    ...
    <section v-else-if="isAccountHash"
      ...
      <div class="text-center d-flex flex-column align-items-center gap-3" style="color:var(--text-primary);">
        <div style="font-size:3rem;">🔒</div>
        <h2 class="fw-black mb-0" style="font-size:1.5rem;">{{ t('account.needLoginTitle') }}</h2>
        <p class="mb-0" style="color:var(--text-secondary);">{{ t('account.needLoginDesc') }}</p>
        <button class="btn btn-warning fw-bold rounded-pill px-4 py-2" @click="goHome">
          {{ t('common.goHome') }}
        </button>
      </div>
    ...
    <div v-else style="min-height:100vh; background:var(--bg-page); color:var(--text-primary); font-family:'Nunito Sans','Segoe UI',sans-serif;">
      ...
      <div class="overflow-x-auto" style="background:var(--bg-page-alt); border-bottom:1px solid var(--border-color-soft); padding:8px 16px;">
        <div class="d-flex gap-4 small fw-bold" style="white-space:nowrap; width:max-content; color:var(--text-secondary);">
      ...
      <div class="container-xl py-3">
          ...
          <div class="col-12 col-xl-7">
            ...
            <div class="row g-2">
              <template v-if="loading">
                <div class="col-4">
                  <div class="p-3 rounded-2 small text-center"
              ...
              <template v-else>
                <div class="col-12">
                  <div class="p-2 rounded-2 small text-center"
                       style="background:var(--bg-card); border:1px solid var(--border-color); color:var(--text-secondary);">
                    {{ t('home.noProducts') }}
                  </div>
                </div>
            ...
            <div class="rounded-3 p-3 h-100 d-flex flex-column gap-3"
              ...
              <div class="d-flex align-items-center gap-2">
                <div class="rounded-circle d-flex align-items-center justify-content-center fw-black flex-shrink-0"
                     style="width:44px; height:44px; background:var(--accent); color:var(--accent-text); font-size:0.8rem;">
                  SAO
                </div>
                <div>
                  <h4 class="fw-black mb-0" style="font-size:0.82rem; line-height:1.3; color:var(--text-heading);">
                    {{ t('home.brandName') }}
                  </h4>
                  <p class="mb-0" style="font-size:10px; color:var(--text-secondary);">{{ t('home.brandTagline') }}</p>
                </div>
              </div>
              <!-- Các liên kết ưu đãi -->
        ...
        <section id="deal-section" class="mt-3">
          ...
          <div class="d-flex gap-2 mb-3 border-bottom pb-0"
            ...
            <button
              v-for="tab in dealTabs" :key="tab.id"
          ...
          <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-2">
            ...
            <div class="d-flex flex-wrap gap-2">
              <button
                v-for="f in dealFilters" :key="f.id"
                class="btn btn-sm fw-bold"
                style="font-size:11px; border-radius:999px;"
                :class="activeFilter === f.id && activeCatId === null
                  ? 'btn-warning text-dark'
                  : 'btn-outline-secondary'"
                :style="!(activeFilter === f.id && activeCatId === null) ? 'color:var(--text-secondary);' : ''"
            ...
            <div class="d-flex align-items-center gap-2">
              <button
                class="btn btn-sm fw-bold"
                style="font-size:11px; border-radius:999px;"
                :class="showAdvFilter ? 'btn-warning text-dark' : 'btn-outline-secondary'"
              ...
              <select v-model="selectedSort"
                      ...
                      style="width:auto; background:var(--bg-input); border-color:var(--border-color-strong); color:var(--text-primary); font-size:12px;"
          ...
          <div v-if="loading" class="text-center py-4 small" style="color:var(--text-secondary);">
            {{ t('home.loadingProducts') }}
          </div>
          <div v-else-if="error" class="alert alert-danger small py-2">{{ error }}</div>
          <div v-else-if="filteredProducts.length === 0" class="text-center py-4 small" style="color:var(--text-secondary);">
            {{ t('home.noMatch') }}
          </div>
        ...
        <div v-if="showCart"
             ...
             style="width:390px; background:var(--bg-page-alt); border-left:1px solid var(--border-color-soft); z-index:500; box-shadow:-12px 0 48px rgba(0,0,0,0.4);">
          ...
          <div class="d-flex justify-content-between align-items-center px-4 py-3" style="border-bottom:1px solid var(--border-color-soft);">
            <div class="d-flex align-items-center gap-2">
              ...
              <span class="fw-bold" style="font-size:0.95rem; color:var(--text-heading);">{{ t('cart.title') }}</span>
          ...
          <div v-if="cartCount === 0" class="flex-grow-1 d-flex flex-column align-items-center justify-content-center gap-3 text-center px-4">
            ...
            <p class="small mb-0" style="color:var(--text-secondary);">{{ t('cart.empty') }}</p>
            <button class="btn btn-sm btn-outline-warning rounded-pill px-4" @click="toggleCart">{{ t('cart.continueShopping') }}</button>
          </div>
  ...
  <Modal v-model="showLoginModal">
    ...
    <div v-if="authTab === 'login' && loginModalErr"
