  ...
  <div>
    <svg v-if="!allZero" :width="width" :height="height" :viewBox="`0 0 ${width} ${height}`" style="width:100%;height:auto;">
      ...
      <g v-for="(bar, i) in bars" :key="i">
        <rect
          :x="bar.slotX" :y="chartTop" :width="slotWidth" :height="chartBottom - chartTop"
          fill="transparent" style="cursor:pointer;"
          @mouseenter="hoverIndex = i" @mouseleave="hoverIndex = null"
        />
        <template v-if="bar.barHeight > 0">
          <rect
            :x="bar.x" :y="bar.y" :width="barWidth" :height="bar.barHeight" rx="4"
            :fill="`url(#${gradientId})`"
            :style="hoverIndex === i ? 'pointer-events:none;filter:brightness(1.2);' : 'pointer-events:none;'"
          />
          <rect
            v-if="bar.barHeight > 4" :x="bar.x" :y="bar.y + bar.barHeight - 4" :width="barWidth" height="4"
            fill="var(--accent-2)"
            :style="hoverIndex === i ? 'pointer-events:none;filter:brightness(1.2);' : 'pointer-events:none;'"
          />
        </template>
      ...
      <g style="pointer-events:none;">
        ...
        <text
          v-if="peakIndex >= 0" :x="bars[peakIndex].slotX + slotWidth / 2" :y="bars[peakIndex].y - 8"
          text-anchor="middle" style="font-size:10px;font-weight:700;fill:var(--text-heading);"
        >★ {{ formatCompact(bars[peakIndex].value) }}</text>
        <text
          v-for="(bar, i) in bars" v-show="i % labelStep === 0" :key="i" :x="bar.slotX + slotWidth / 2" :y="height - 4"
          text-anchor="middle" style="font-size:9px;fill:var(--text-secondary);"
        >{{ bar.label }}</text>
      </g>
    </svg>
