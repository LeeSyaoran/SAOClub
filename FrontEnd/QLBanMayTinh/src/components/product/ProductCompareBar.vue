<template>
  <!-- Thanh so sánh nổi ở đáy màn hình — chỉ hiện khi đã chọn ít nhất 1 sản phẩm -->
  <div v-if="items.length > 0"
    ...
    <div class="d-flex gap-2 flex-grow-1" style="overflow-x:auto;">
      <div v-for="item in items" :key="item.bienTheId"
        ...
        <button class="btn-close btn-close-white" style="font-size:8px;"
                :aria-label="t('common.remove')"
                @click="$emit('remove', item)"></button>
      </div>
    </div>
    ...
    <button class="btn btn-sm btn-outline-secondary flex-shrink-0" style="font-size:11px;"
            @click="$emit('clear')">
      {{ t('productCompare.barClear') }}
    </button>
    <button class="btn btn-sm btn-warning text-dark fw-bold flex-shrink-0" style="font-size:11px;"
